...
export type DrawerTriggerProps<T extends ValidComponent = "button"> = ComponentProps<typeof DrawerPrimitive.Trigger<T>>;

export const DrawerTrigger = <T extends ValidComponent = "button">(props: DrawerTriggerProps<T>) => {
  return <DrawerPrimitive.Trigger data-slot="drawer-trigger" {...props} />;
};

export type DrawerCloseProps<T extends ValidComponent = "button"> = ComponentProps<typeof DrawerPrimitive.Close<T>>;

export const DrawerClose = <T extends ValidComponent = "button">(props: DrawerCloseProps<T>) => {
  return <DrawerPrimitive.Close data-slot="drawer-close" {...props} />;
};

export type DrawerContentProps<T extends ValidComponent = "div"> = ComponentProps<typeof DrawerPrimitive.Content<T>> & {
  withHandle?: boolean;
};
    ...
    <>
      ...
      <DrawerPrimitive.Content
        ...
        class={cx(
          ...
          context.side() === "top" && ["inset-x-0 top-0 mb-24 max-h-[80vh] rounded-b-lg border-b", "after:inset-x-0 after:bottom-[calc(100%-1px)] after:h-1/2"],
          context.side() === "left" && ["inset-y-0 left-0 w-3/4 border-r sm:max-w-sm", "after:inset-y-0 after:right-[calc(100%-1px)] after:w-1/2"],
      ...
      >
        <Show when={props.withHandle}>
          <div class={cx("bg-muted shrink-0 self-center rounded-full", context.side() === "bottom" && "mt-4 h-1 w-10")} />
        </Show>
        {props.children}
      </DrawerPrimitive.Content>
    </>
...
export type DrawerLabelProps<T extends ValidComponent = "h2"> = ComponentProps<typeof DrawerPrimitive.Label<T>>;
